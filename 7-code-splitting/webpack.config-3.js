...
/** 静态导入 和 动态导入的结合
 *    静态导入是最常用的方式，方便阅读和维护
 *    动态导入可以处理一些特殊的场景，优化性能
 *       --- 懒加载 | 预加载 
 *          （点击某个按钮触发的时候才会去加载 js， 如果js 文件过大，页面会卡顿）
 *       
 *       --- 预获取 
 *          （浏览器会先发送一个 预获取请求，在浏览器资源空闲的时候，将该文件下载下来，
 *           等到用户实际去点击按钮触发的时候，才会去加载这个js，可以优化首屏性能）
 */
module.exports = {
    // entry: './src/index.js',

    // 多入口 （既使用 静态导入 又使用 动态导入的）
    //   ---- 还是需要使用 optimization.splitChunks 自动分离出公共代码（针对静态导入的）
    //   ---- 代码中也可以使用 动态导入 import('').then  的方式
    entry: {
        index: './src3/index.js',
        another: './src3/another.js'
    },
